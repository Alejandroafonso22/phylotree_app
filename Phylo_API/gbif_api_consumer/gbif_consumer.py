...
def gbif_consumer_master(scientific_name: str, api_specie_id: int = 2, api_user_id: Union[int, None]= None):
    ...
    if(days_from_download.days > 60 or days_from_download.days == 0):
        ...
        taxon_key = getGbifTaxonId(scientific_name)

        if taxon_key:
            resultSet = getSpecieOccurrences(taxon_key, api_specie_id, api_user_id, ocurrences_tidy)
        ...
        return saveData(taxon_occurrences, download_id, api_specie_id)

    else:
        return {"status": "recently_downloaded"}

def saveData(taxon_occurrences: pd.DataFrame, download_id: str, specie_id: int):
...
def getGbifTaxonId(scientific_name: str) -> int:
...
def getSpecieOccurrences(taxon_key: int, specie_id: int, user_id: Union[int, None], curr_occurrences: list) -> list:
    ...
    try:
        ...
        if "getSpecieOccurrences" == inspect.trace()[-1][3]:
